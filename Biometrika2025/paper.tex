...
\begin{proof}
By Assumption~4, $n^{1/2}(T_{i,n} - T_{j,n})$ is asymptotically normal with mean $n^{1/2}(T_i^\infty - T_j^\infty)$ and variance $\sigma_{ij}^2$. Standardizing gives $Z_{ij}^{(n)} \xrightarrow{d} N(\kappa_{ij}, 1)$. The Gaussian tail bound is standard.
\end{proof}
...
\subsection{Example 2: AIC for nested models}
...
\begin{table}[ht]
\tbl{AIC comparison: restricted ($s=3$) versus full ($p$) models, $n=500$}
{\begin{tabular}{crr}
...
\section{Example 1: Conformal prediction width}
...
Under block-equicorrelated predictors, $\partial\kappa/\partial\rho < 0$ for sets containing both linear and tree-based estimators, implying $\partial\Delta_n/\partial\rho > 0$. For purely linear sets, the variation in $\hat\kappa$ across $\rho$ is within the Monte Carlo noise range (Table~6), consistent with $\partial\kappa/\partial\rho \approx 0$ at the population level.
\end{corollary}
...
$\rho$ & $\hat R_{\text{OLS}}$ & $\hat R_{\text{RF}}$ & $\hat\kappa$ & $\Delta$ & SE \\ \hline
...
The plug-in estimator $\hat\kappa$ targets $\sqrt{n_{\text{cal}}}\cdot\kappa$. From calibration residuals:
\begin{enumerate}
...
{\begin{tabular}{cccccc}
$\rho$ & $\hat R_{\text{OLS}}$ & $\hat R_{\text{Ridge}}$ & $\hat\kappa$ & $\Delta$ & SE \\ \hline
...
\textbf{Training MSE as selection functional.}
\begin{table}[h!]
\small
